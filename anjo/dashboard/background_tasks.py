...
# Bounded thread pool for background tasks (facts extraction + mid-reflect).
# Max 4 workers: prevents unbounded thread creation under load while still
# allowing concurrent background work across multiple users.
_bg_executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="anjo-bg")
    ...
    def _run():
        try:
            ...
            if not isinstance(text_attr, str):
                logger.error(f"Quick facts extraction: unexpected LLM response (no text) for {user_id}: {response}")
                return
            raw = re.sub(r"^```(?:json)?\s*", "", text_attr.strip())
            raw = re.sub(r"\s*```$", "", raw)
