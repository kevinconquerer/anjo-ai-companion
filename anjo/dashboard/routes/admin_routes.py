...
async def admin_users(request: Request, page: int = 1, limit: int = 100):
    ...
    return {
        ...
        "subscribers": sum(1 for r in rows if r["tier"] not in {"free", "self-hosted"}),
...
async def admin_add_credits(user_id: str, request: Request, amount: float = 5.0):
    if not _authorized(request):
        return _unauth()
    return JSONResponse({"detail": "Billing not available in open-source build"}, status_code=501)
