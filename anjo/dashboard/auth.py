...
def validate_username(username: str) -> str | None:
    """Return an error message if the username is invalid, or None if it passes."""
...
def load_revoked_tokens_from_db() -> None:
    ...
    now = int(time.time())
    try:
        rows = get_db().execute("SELECT sig, exp FROM revoked_tokens WHERE exp > ?", (now,)).fetchall()
        for row in rows:
            _revoked_tokens.add((row["sig"], row["exp"]))
    except Exception:
        logger.exception("Failed to load revoked tokens from DB; in-memory revocation set may be stale")
...
def force_verify_email(username: str) -> None:
    ...
    db = get_db()
    db.execute(
        "UPDATE users SET email_verified = 1, verification_token = '', verification_token_hmac = '' WHERE username = ?",
        (username,),
    )
    db.commit()
...
def is_email_verified(user_id: str) -> bool:
    row = (
        get_db()
        .execute(
            "SELECT email_verified, verification_token FROM users WHERE user_id = ?",
            (user_id,),
        )
        .fetchone()
    )
...
def verify_password(user_id: str, password: str) -> bool:
    row = get_db().execute("SELECT hashed_password FROM users WHERE user_id = ?", (user_id,)).fetchone()
    if not row:
        return False
    try:
...
def list_users() -> list[dict]:
    ...
    rows = (
        get_db()
        .execute("SELECT user_id, username, email, email_verified, created_at FROM users ORDER BY created_at DESC")
        .fetchall()
    )
...
def verify_token(token: str) -> str | None:
    ...
    try:
        row = get_db().execute("SELECT password_changed_at FROM users WHERE user_id = ?", (user_id,)).fetchone()
        if row is None:
