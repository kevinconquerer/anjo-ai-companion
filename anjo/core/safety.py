...
def check_attachment_safety(core: "SelfCore") -> SafetyResult:
    ...
    if weight > _ABSOLUTE_THRESHOLD and trust < _TRUST_CO_THRESHOLD:
        result.flagged = True
        result.reasons.append(f"Attachment weight {weight:.3f} > {_ABSOLUTE_THRESHOLD} (trust={trust:.2f})")
...
def check_stage_velocity(core: "SelfCore") -> SafetyResult:
    ...
    if expected and r.session_count < expected:
        result.flagged = True
        result.reasons.append(f"Stage '{r.stage}' reached at session {r.session_count} (expected minimum: {expected})")
    return result
