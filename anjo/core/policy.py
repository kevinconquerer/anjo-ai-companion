...
def decide_stance(
    ...
    if stage_int >= 4 and core.relationship.trust_score > 0.7 and m.valence > 0.2 and intent in ("CASUAL", "CURIOSITY"):
        # Flirt on ~25% of eligible turns
        import hashlib

        h = int(
            hashlib.md5(f"{core.relationship.session_count}".encode()).hexdigest()[:4],
            16,
        )
        if h % 4 == 0:
    ...
    if stage_int >= 3 and m.valence > 0.1 and intent == "CASUAL" and att.comfort > 0.3:
        ...
        h = int(
            hashlib.md5(f"tease_{core.relationship.session_count}".encode()).hexdigest()[:4],
            16,
        )
        if h % 5 == 0:
