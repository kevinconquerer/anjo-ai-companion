...
def verify_production_key() -> None:
    ...
    if secret and len(secret) < 32:
        ...
        logging.getLogger(__name__).warning("⚠️  ANJO_SECRET is short (%d chars). Minimum 32 recommended.", len(secret))
        return
    # No secret set
    if os.environ.get("ANJO_ENV", "") != "dev":
    ...
    if _users_dir.exists() and any(_users_dir.iterdir()):
        ...
        logging.getLogger(__name__).warning(
            "⚠️  Using dev fallback encryption key with existing user data. Set ANJO_SECRET for production use."
        )
