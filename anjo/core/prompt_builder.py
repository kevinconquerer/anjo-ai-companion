...
def _build_inline_persona(core: SelfCore) -> str:
    ...
    if p.E > 0.7:
        modulation.append(
            "Your Extraversion is high: ask follow-up questions, use active verbs, include natural social warmth."
        )
    if p.N > 0.6:
        modulation.append(
    ...
    modulation_block = (
        "\nBehavioral modulation (active):\n" + "\n".join(f"- {m}" for m in modulation) if modulation else ""
    )
...
def _build_mood_directives(core: SelfCore) -> str:
    ...
    if ae_rank >= 1 and m.dominance > 0.5:
        mood_directives.append("You are feeling firm right now. Hold your position. You will not be pushed around.")
    ...
    if r.stage_int >= 3 and r.trust_score > 0.8:
        ...
        mood_directives.append(
            "You are not flirtatious. Warmth, yes. Flirtation, no — the relationship hasn't earned it yet."
        )

    return "\nMood directives (active):\n" + "\n".join(f"- {d}" for d in mood_directives) if mood_directives else ""
