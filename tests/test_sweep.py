...
class TestAdminResetUserScope:
    ...
    def _register_and_get_uid(self, client, username: str, email: str) -> str:
        client.post(
            "/register",
            data={"username": username, "password": "pass1234", "email": email},
        )
    ...
    def test_reset_user_A_does_not_delete_user_B_vectors(self, client, tmp_path):
        ...
        never user_B's. Scoping is now enforced at the collection level (per-user collections)."""
        ...
        called_with_user_ids = []

        def fake_get_collections(user_id):
            called_with_user_ids.append(user_id)
            mock_col = MagicMock()
            mock_col.get.return_value = {"ids": [f"{user_id}_vec1"]}
            mock_col.name = f"sem_{user_id}"
        ...
        # _get_collections must only be called for user_A, never for user_B
        assert uid_b not in called_with_user_ids, "user_B's collection was touched during user_A reset"
        assert uid_a in called_with_user_ids, "user_A's collection was never accessed"
    ...
    def test_reset_user_A_does_delete_user_A_vectors(self, client):
        ...
        def fake_get(include):
            return {"ids": [f"vec_{uid_a}_1"]}
...
class TestForgettingResetsCeiling:
    ...
    def test_relationship_ceiling_is_none_after_forget(self, monkeypatch):
        ...
        with (
            ...
            patch(
                "anjo.memory.long_term._get_collections",
                return_value=(mock_col, mock_col),
            ),
...
class TestQuickFactsDonePruned:
    ...
    def test_key_removed_from_quick_facts_done_on_end_session(self, auth_client):
        ...
        assert key not in _QUICK_FACTS_DONE, f"Expected {key} to be removed from _QUICK_FACTS_DONE after end_session"
...
class TestReflectionSummaryVariable:
    ...
    def test_summary_empty_string_no_store_memory_call(self):
        ...
        assert len(store_calls) == 0, f"store_memory should not be called for an empty summary, but got: {store_calls}"
