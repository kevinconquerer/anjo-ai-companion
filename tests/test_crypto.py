...
def test_register_stores_email_encrypted_and_hmac(client):
    ...
    row = get_db().execute("SELECT email, email_hmac FROM users WHERE username = ?", ("cryptouser",)).fetchone()
...
def test_facts_stored_encrypted(client):
    ...
    row = get_db().execute("SELECT facts_json FROM facts WHERE user_id = ?", ("user_enc_test",)).fetchone()
    assert row is not None
